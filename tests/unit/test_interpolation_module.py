...
def test_validator_missing_xk():
    ...
    with pytest.raises(ValidationError):
        NumericalMethod(
            method="interpolation",
            input_data={"mode": "table", "calculation_mode": "lagrange", "data": df},
        ).validate_input()
...
def test_newton_executor():
    ...
    method = NumericalMethod(
        method="interpolation",
        input_data={"mode": "table", "calculation_mode": "newton", "data": df, "xk": 1},
    )
...
def test_hermite_executor():
    df = pd.DataFrame({"x": [0, 1], "f(x)": [1, 2], "f'(x)": [0, 1]})
